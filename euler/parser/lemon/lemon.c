...
static int lemon_vsprintf(char *str, const char *zFormat, va_list ap)
{
	...
	for (i = j = 0; (c = zFormat[i]) != 0; i++) {
		if (c == '%') {
			...
			c = zFormat[++i];
			if (ISDIGIT(c) ||
			    (c == '-' && ISDIGIT(zFormat[i + 1]))) {
				if (c == '-')
					i++;
				while (ISDIGIT(zFormat[i]))
					iWidth = iWidth * 10 + zFormat[i++] -
						 '0';
				if (c == '-')
					iWidth = -iWidth;
				c = zFormat[i];
			...
			if (c == 'd') {
				...
				while (v > 0) {
					k++;
					zTemp[sizeof(zTemp) - k] =
						(v % 10) + '0';
					v /= 10;
				}
				lemon_addtext(str, &nUsed,
					      &zTemp[sizeof(zTemp) - k], k,
					      iWidth);
			} else if (c == 's') {
				z = va_arg(ap, const char *);
				lemon_addtext(str, &nUsed, z, -1, iWidth);
			} else if (c == '.' &&
				   memcmp(&zFormat[i], ".*s", 3) == 0) {
				i += 2;
				k = va_arg(ap, int);
				z = va_arg(ap, const char *);
...
struct state {
	...
	int nTknAct,
		nNtAct; /* Number of actions on terminals and nonterminals */
	int iTknOfst,
		iNtOfst; /* yy_action[] offset for terminals and nonterms */
...
static struct action *Action_new(void)
{
	...
	if (actionfreelist == 0) {
		...
		int amt = 100;
		actionfreelist =
			(struct action *)calloc(amt, sizeof(struct action));
		if (actionfreelist == 0) {
			fprintf(stderr,
	...
	for (i = p->nAction - 1; i >= end; i--) {
      ...
      ** must match against the candidate aAction[i] entry. */
			...
			for (j = 0; j < p->nLookahead; j++) {
				k = p->aLookahead[j].lookahead -
				    p->mnLookahead + i;
				if (k < 0 || k >= p->nAction)
					break;
				if (p->aLookahead[j].lookahead !=
				    p->aAction[k].lookahead)
					break;
				if (p->aLookahead[j].action !=
				    p->aAction[k].action)
					break;
			}
			if (j < p->nLookahead)
...
symbol instead.",
  ...
  ** start symbol in this case.) */
	for (rp = lemp->rule; rp; rp = rp->next) {
		...
		for (i = 0; i < rp->nrhs; i++) {
			if (rp->rhs[i] ==
			    sp) { /* FIX ME:  Deal with multiterminals */
				ErrorMsg(
					lemp->filename, 0,
...
PRIVATE struct state *getstate(struct lemon *lemp)
{
  ...
  ** by prior calls to "Configlist_addbasis()". */
    ...
    ** preexisting state, then return a pointer to the preexisting state */
		...
		Configlist_closure(
			lemp); /* Compute the configuration closure */
		...
		stp->statenum =
			lemp->nstate++; /* Every state gets a sequence number */
		stp->ap = 0; /* No actions, yet. */
		State_insert(stp, stp->bp); /* Add to the state table */
		buildshifts(lemp,
			    stp); /* Recursively compute successor states */
	}
	return stp;
}
    ...
    ** on the symbol "sp" */
		if (sp->type == MULTITERMINAL) {
			...
			for (i = 0; i < sp->nsubsym; i++) {
				Action_add(&stp->ap, SHIFT, sp->subsym[i],
					   (char *)newstp);
			}
		} else {
  ...
  */
	for (i = 0; i < lemp->nstate; i++) { /* Loop over all states */
		...
		for (cfp = stp->cfp; cfp;
			...
			if (cfp->rp->nrhs ==
			    cfp->dot) { /* Is dot at extreme right? */
				for (j = 0; j < lemp->nterminal; j++) {
					if (SetFind(cfp->fws, j)) {
						/* Add a reduce action to the state "stp" which will reduce by the
            ** rule "cfp->rp" if the lookahead symbol is "lemp->symbols[j]" */
						Action_add(&stp->ap, REDUCE,
							   lemp->symbols[j],
							   (char *)cfp->rp);
					}
				}
  ...
  ** start nonterminal.  */
	...
	for (i = 0; i < lemp->nstate; i++) {
		...
		for (ap = stp->ap; ap && ap->next; ap = ap->next) {
			for (nap = ap->next; nap && nap->sp == ap->sp;
			     nap = nap->next) {
...
int main(int argc, char **argv)
{
	...
	static struct s_options options[] = {
		{ OPT_FLAG, "b", (char *)&basisflag,
		  "Print only the basis in report." },
		{ OPT_FLAG, "c", (char *)&compress,
		...
		{ OPT_FSTR, "f", 0,
		  "Ignored.  (Placeholder for -f compiler options.)" },
		{ OPT_FLAG, "g", (char *)&rpflag,
		...
		{ OPT_FLAG, "q", (char *)&quiet,
		  "(Quiet) Don't print the report file." },
		{ OPT_FLAG, "r", (char *)&noResort,
		  "Do not sort or renumber states" },
		{ OPT_FLAG, "s", (char *)&statistics,
		  "Print parser stats to standard output." },
		{ OPT_FLAG, "x", (char *)&version,
		  "Print the version number." },
		{ OPT_FSTR, "T", (char *)handle_T_option,
...
static int handleflags(int i, FILE *err)
{
	...
	for (j = 0; op[j].label; j++) {
		if (strncmp(&g_argv[i][1], op[j].label,
			    lemonStrlen(op[j].label)) == 0)
			break;
	}
	v = g_argv[i][0] == '-' ? 1 : 0;
	if (op[j].label == 0) {
...
to follow the previous rule.");
	...
	case IN_RHS:
		if (x[0] == '.') {
			...
			if (msp->type != MULTITERMINAL) {
				...
				msp->nsubsym = 1;
				msp->subsym = (struct symbol **)calloc(
	...
	case WAITING_FOR_DECL_ARG:
		if (x[0] == '{' || x[0] == '\"' || ISALNUM(x[0])) {
			...
			addLineMacro = !psp->gp->nolinenosflag &&
				       psp->insertLineMacro &&
				       (psp->decllinenoslot == 0 ||
					psp->decllinenoslot[0] != 0);
			if (addLineMacro) {
				for (z = psp->filename, nBack = 0; *z; z++) {
					if (*z == '\\')
						nBack++;
				}
				lemon_sprintf(zLine, "#line %d ",
					      psp->tokenlineno);
				nLine = lemonStrlen(zLine);
				n += nLine + lemonStrlen(psp->filename) + nBack;
			}
			*psp->declargslot =
				(char *)realloc(*psp->declargslot, n);
	...
	case WAITING_FOR_TOKEN_NAME:
		...
		if (x[0] == '.') {
			...
			ErrorMsg(psp->filename, psp->tokenlineno,
				 "%%token argument \"%s\" should be a token",
				 x);
			psp->errorcnt++;
		} else {
			(void)Symbol_new(x);
	...
	case WAITING_FOR_WILDCARD_ID:
		if (x[0] == '.') {
			...
			ErrorMsg(psp->filename, psp->tokenlineno,
				 "%%wildcard argument \"%s\" should be a token",
				 x);
			psp->errorcnt++;
		} else {
			struct symbol *sp = Symbol_new(x);
	...
	case WAITING_FOR_CLASS_ID:
		if (!ISLOWER(x[0])) {
			ErrorMsg(
				psp->filename, psp->tokenlineno,
				"%%token_class must be followed by an identifier: %s",
				x);
			psp->errorcnt++;
			psp->state = RESYNC_AFTER_DECL_ERROR;
	...
	case WAITING_FOR_CLASS_TOKEN:
		if (x[0] == '.') {
			...
			ErrorMsg(
				psp->filename, psp->tokenlineno,
				"%%token_class argument \"%s\" should be a token",
				x);
			psp->errorcnt++;
			psp->state = RESYNC_AFTER_DECL_ERROR;
		}
	...
	for (i = 0; z[i]; i++) {
		...
		if (strncmp(&z[i], "%endif", 6) == 0 && ISSPACE(z[i + 6])) {
			...
			if (exclude) {
				...
				for (n = 0; z[j + n] && !ISSPACE(z[j + n]);
				     n++) {
				}
				exclude = 1;
				for (k = 0; k < nDefine; k++) {
					if (strncmp(azDefine[k], &z[j], n) ==
						    0 &&
...
void Parse(struct lemon *gp)
{
	...
	if (fread(filebuf, 1, filesize, fp) != filesize) {
		ErrorMsg(ps.filename, 0,
			 "Can't read in all %d bytes of this file.", filesize);
		free(filebuf);
		gp->errorcnt++;
		fclose(fp);
		return;
...
struct plink *Plink_new(void)
{
	...
	if (plink_freelist == 0) {
		...
		int amt = 100;
		plink_freelist =
			(struct plink *)calloc(amt, sizeof(struct plink));
		if (plink_freelist == 0) {
			fprintf(stderr,
...
)
{
	...
	case REDUCE: {
		...
		fprintf(fp, "%*s reduce       %-7d", indent, ap->sp->name,
			rp->iRule);
		RulePrint(fp, rp, -1);
		break;
	}
	case SHIFTREDUCE: {
		struct rule *rp = ap->x.rp;
		fprintf(fp, "%*s shift-reduce %-7d", indent, ap->sp->name,
			rp->iRule);
		RulePrint(fp, rp, -1);
		break;
	}
	...
	if (result && ap->spOpt) {
		fprintf(fp, "  /* because %s==%s */", ap->sp->name,
			ap->spOpt->name);
	}
	return result;
}
...
PRIVATE char *pathsearch(char *argv0, char *name, int modemask)
{
	...
	if (cp) {
		c = *cp;
		*cp = 0;
		path = (char *)malloc(lemonStrlen(argv0) + lemonStrlen(name) +
				      2);
		if (path)
		...
		*cp = c;
		...
		path = (char *)malloc(lemonStrlen(pathlist) +
				      lemonStrlen(name) + 2);
		if ((pathbuf != 0) && (path != 0)) {
			pathbufptr = pathbuf;
...
PRIVATE int translate_code(struct lemon *lemp, struct rule *rp)
{
	...
	if (lhsdirect) {
		sprintf(zLhs, "yymsp[%d].minor.yy%d", 1 - rp->nrhs,
			rp->lhs->dtnum);
	} else {
		rc = 1;
	...
	for (i = 0; i < rp->nrhs; i++) {
		if (rp->rhsalias[i]) {
			if (i > 0) {
				int j;
				if (rp->lhsalias &&
				    strcmp(rp->lhsalias, rp->rhsalias[i]) ==
					    0) {
					ErrorMsg(
						lemp->filename, rp->ruleline,
...
)
{
	...
	fprintf(out, "static const %s yy_shift_ofst[] = {\n",
		minimum_size_type(mnTknOfst, lemp->nterminal + lemp->nactiontab,
				  &sz));
	lineno++;
	lemp->tablesize += n * sz;
	for (i = j = 0; i < n; i++) {
	...
	if (lemp->tokendest) {
		...
		for (i = 0;
		     i < lemp->nsymbol && lemp->symbols[i]->type != TERMINAL;
		     i++)
			;
		if (i < lemp->nsymbol) {
			emit_destructor_code(out, lemp->symbols[i], lemp,
					     &lineno);
			fprintf(out, "      break;\n");
			lineno++;
		}
...
void ReportHeader(struct lemon *lemp)
{
	...
	if (in) {
		int nextChar;
		for (i = 1; i < lemp->nterminal && fgets(line, LINESIZE, in);
		     i++) {
	...
	for (i = 0; i < lemp->nstate; i++) {
		...
		for (ap = stp->ap; ap; ap = ap->next) {
			...
			if (ap->type != SHIFT)
				continue;
			pNextState = ap->x.stp;
			if (pNextState->autoReduce &&
			    pNextState->pDfltReduce != 0) {
				ap->type = SHIFTREDUCE;
...
void ResortStates(struct lemon *lemp)
{
	...
	while (lemp->nxstate > 1 &&
	       lemp->sorted[lemp->nxstate - 1]->autoReduce) {
		lemp->nxstate--;
	}
}
...
void Strsafe_init(void)
{
	...
	if (x1a) {
		x1a->size = 1024;
		x1a->count = 0;
		x1a->tbl = (x1node *)calloc(1024,
					    sizeof(x1node) + sizeof(x1node *));
		if (x1a->tbl == 0) {
			free(x1a);
			x1a = 0;
		} else {
...
int Strsafe_insert(const char *data)
{
	...
	if (x1a->count >= x1a->size) {
		...
		array.tbl = (x1node *)calloc(arrSize,
					     sizeof(x1node) + sizeof(x1node *));
		if (array.tbl == 0)
...
void Symbol_init(void)
{
	...
	if (x2a) {
		x2a->size = 128;
		x2a->count = 0;
		x2a->tbl = (x2node *)calloc(128,
					    sizeof(x2node) + sizeof(x2node *));
		if (x2a->tbl == 0) {
			free(x2a);
			x2a = 0;
		} else {
...
int Symbol_insert(struct symbol *data, const char *key)
{
	...
	if (x2a->count >= x2a->size) {
		...
		array.tbl = (x2node *)calloc(arrSize,
					     sizeof(x2node) + sizeof(x2node *));
		if (array.tbl == 0)
			return 0; /* Fail due to malloc failure */
...
void State_init(void)
{
	...
	if (x3a) {
		x3a->size = 128;
		x3a->count = 0;
		x3a->tbl = (x3node *)calloc(128,
					    sizeof(x3node) + sizeof(x3node *));
		if (x3a->tbl == 0) {
			free(x3a);
			x3a = 0;
		} else {
...
int State_insert(struct state *data, struct config *key)
{
	...
	if (x3a->count >= x3a->size) {
		...
		array.tbl = (x3node *)calloc(arrSize,
					     sizeof(x3node) + sizeof(x3node *));
		if (array.tbl == 0)
			return 0; /* Fail due to malloc failure */
...
void Configtable_init(void)
{
	...
	if (x4a) {
		...
		x4a->count = 0;
		x4a->tbl =
			(x4node *)calloc(64, sizeof(x4node) + sizeof(x4node *));
		if (x4a->tbl == 0) {
			free(x4a);
			x4a = 0;
		} else {
...
int Configtable_insert(struct config *data)
{
	...
	np = x4a->ht[h];
	while (np) {
		if (Configcmp((const char *)np->data, (const char *)data) ==
		    0) {
			/* An existing entry with the same key is found. */
			/* Fail because overwrite is not allows. */
			return 0;
	...
	if (x4a->count >= x4a->size) {
		...
		array.tbl = (x4node *)calloc(arrSize,
